...
import { VariableAlias } from './variables.js'
...
export type ImageType = 'JPG' | 'PNG' | 'SVG' | 'PDF'
...
export type LayoutGridAlignment = 'MIN' | 'MAX' | 'CENTER'
...
export type PaintSolidScaleMode = 'FILL' | 'FIT' | 'TILE' | 'STRETCH'
...
export type PathWindingRule = 'EVENODD' | 'NONZERO'
...
export interface ComponentSet {
  ...
  /** Data on component set's containing page, if component set resides in a multi-page file */
...
export type LineTypes = 'ORDERED' | 'UNORDERED' | 'NONE'
...
export type Node<NType extends NodeType = NodeType> = {
   ...
   * Most fields will only map to a single `VariableAlias` However, for fills, strokes, size, and component properties, it is possible to have multiple variables bound to the field.
   * @beta
   * */
...
export type FrameOffsetRegion = RegionBase & {
  /** Unique id specifying the frame. */
  node_id: string
  /** 2D vector offset within the frame. */
  node_offset: Vector
